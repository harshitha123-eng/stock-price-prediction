...
def analyze_stock_data(company_name, start_date, end_date):
    ...
    filtered_news_df = stock_news_df[
        (stock_news_df['CompanyName'].str.contains(company_name, case=False, na=False)) &
        (stock_news_df['Date'] >= start_date) &
        (stock_news_df['Date'] <= end_date)
    ]

    if filtered_news_df.empty:
        print("No matching data found. Check the following:")
        print("Unique Stock Names:", stock_news_df['CompanyName'].unique())
        print("Date Range in Data:", stock_news_df['Date'].min(), "-", stock_news_df['Date'].max())
        print(f"Filter Inputs: company_name={company_name}, start_date={start_date}, end_date={end_date}")
    ...
    if not filtered_news_df.empty:
        filtered_news_df['Polarity'] = filtered_news_df['Tweet'].apply(lambda tweet: TextBlob(tweet).sentiment.polarity)
        positive_tweets = (filtered_news_df['Polarity'] > 0).sum()
        negative_tweets = (filtered_news_df['Polarity'] < 0).sum()
        neutral_tweets = (filtered_news_df['Polarity'] == 0).sum()
